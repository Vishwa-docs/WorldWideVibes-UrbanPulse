...
- **httpx** — Async HTTP client (for Bright Data)
...
# Required for live web scraping (optional — simulated mode works without it)
...
> **Tip**: The app runs fully functional in demo mode without any API keys.
> Gemini falls back to template-based responses and Bright Data uses simulated data.
...
The app uses **Gemini 2.0 Flash** for:
...
**Without a key**: The `FallbackLLMService` generates template-based responses
...
UrbanPulse uses the **Web Scraper API** with the Google Maps dataset (`gd_l1vijqt9jfj7olhj`) to fetch:
- Points of Interest near properties
- Business reviews and ratings
- Competition density

**Without a token**: The `BrightDataClient` generates realistic simulated data
using deterministic randomness based on coordinates, so the full scoring pipeline
works identically.
...
- `test_brightdata.py` — Bright Data client
...
### Bright Data returning simulated data
